# Zz(Zero Trust, Zero Governance, Peer to Peer, Secured, Chatting Software, Serving on Mobile Phone Only System)

## 什么是Zz软件

Zz是指在保护个人隐私的点对点安全聊天软件。

它的主要特点：

1. 无信任，无管制(Zero True, Zero Governance)
2. 基于IP访问技术而非DNS技术，抵抗域名污染
3. 采用类似加密币形式的地址(Address)进行节点认识与校验，从而对于节点的IP变动不敏感
4. 协议主要基于Web技术，包括HTTP, WebRTC等
5. 聊天服务器完全位于移动端，没有中间大型集中服务器
6. 推广并支持基于HTTP的Web App技术
...
出于安全考量，软件的聊天内容的重要信息都采用了*非对称加密*或者*对称加密*。
对称加密用于加密大量的数据，非对称加密用于加密少量的数据。
加密原理如下：

1. 先是两个节点交换公钥，并对过签名校验私钥，从而完成公钥的匹配。
2. 然后通过协议指定*对称加密*算法并随机生成一个密钥，分别通过公钥加密。
3. 通过指定的加密算法与密钥，加密发送的消息内容，并将加密后的数据发送到对应节点。 
4. 节点接收后，通过*非对称加密*私钥解压，获得*对称加密*算法和密钥。通过获得的密钥与算法，解密发送的消息。
5. 完成整个加解密过程
...
#### 私钥
是最核心的字符串，必须妥善保管，丢失后会帐号将无法恢复。

### 地址
地址是一串用于识别与校验好友的字符串。它是由非对称加密算法的公钥哈希后得到的一串字符串。
地址上可以绑定用户的所有信息。

## IP与端口
IP与端口是TCP/IP协议连接的基本协议对。
是最成熟，最广泛的广域网联网技术，是互联网技术的基础结构，俗称插座(Socket）技术。
其中
1. IP
用于定位网络机器。目前有两个版本IP V4与 IP V6。 
V4公网IP数量相对受限，主要提供网站使用
V6公网IP数量庞大，将会被广泛应用于各种设备
本软件应用的基础与前提是IPV6的成熟与广泛应用

2. 端口
用于区分软件服务。比如(ssh的默认端口是22, ftp服务的默认端口是21，网站服务的默认端口是80)
